...
def start_functions_keyboard():
    ...
    keyboard.add(InlineKeyboardButton(text="🧠 Чем полезна игра Го?", callback_data="benefits_of_go"))  # Преимущества игры
...
def return_from_branches_functions_keyboard():
    keyboard = InlineKeyboardBuilder()
    keyboard.add(InlineKeyboardButton(text="🔎 Посмотреть на 2ГИС 🌍", url="https://2gis.kg/bishkek/search/sengoku.kg/firm/70000001060100301/74.620524%2C42.827672?m=74.612824%2C42.824463%2F12.04"))
    keyboard.add(InlineKeyboardButton(text="Вернуться в главное меню 🏠", callback_data="start"))
    return keyboard.adjust(1).as_markup()
...
def contact_us_functions_keyboard():
    ...
    keyboard.add(InlineKeyboardButton(text="📷 Наш Instagram", url="https://www.instagram.com/sengoku.kg/"))
    keyboard.add(InlineKeyboardButton(text="📱 Написать в WhatsApp", url="https://api.whatsapp.com/send/?phone=996509240706"))
